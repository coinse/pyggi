...
class AbstractProgram(ABC):
    ...
    def remove_tmp_variant(self):
        ...
        try:
            os.rmdir(os.path.join(self.__class__.TMP_DIR, self.name))
            os.rmdir(self.__class__.TMP_DIR)
        except OSError as e:
    ...
    def exec_cmd(self, cmd, timeout=15, env=None):
        sprocess = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
        try:
            start = time.time()
